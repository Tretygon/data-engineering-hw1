...
def main(outp_path='./'):
    ...
    with open(f'{outp_path}care_providers.ttl','wb') as f:
        f.write(data_cube.serialize(format="ttl",encoding='utf-8'))
    run_constraint_checks(data_cube)


def get_data():
    with requests.get('https://data.mzcr.cz/distribuce/63/narodni-registr-poskytovatelu-zdravotnich-sluzeb.csv', stream=True) as r:
        cols = {'KrajCode':str,'OkresCode':str,'DruhZarizeni':str}
        df = pd.read_csv(StringIO(r.content.decode('utf-8')),usecols=cols.keys(),converters=cols)
    
    df = df.groupby(list(cols.keys())).size().reset_index(name ='Count')
    return df
